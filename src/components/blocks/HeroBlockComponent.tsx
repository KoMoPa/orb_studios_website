...
export default function HeroBlockComponent({
  ...
  const imageUrl = `/media/${backgroundImage.filename}`
  const overlayClass = overlay === 'dark' ? 'bg-black/50' : overlay === 'light' ? 'bg-white/20' : ''
  ...
  return (
    ...
    >
      ...
      <div className="relative z-20 text-center text-white px-4 max-w-5xl mx-auto animate-fadeInUp">
        <h1 className={`${titleFont} text-5xl font-bold uppercase text-white mb-4 drop-shadow-lg tracking-wider`}>
          {title}
        </h1>
        <h2 className="text-4xl font-semibold mb-6 drop-shadow-md">{subtitle}</h2>

        {description && <p className="text-lg mb-8 max-w-2xl mx-auto drop-shadow-md">{description}</p>}
